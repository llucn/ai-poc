...
当服务端解析到 LLM 返回的 action 中 tool 名称以 `client__` 开头时，服务端 MUST NOT 在服务端执行，而是 MUST 挂起当前 LLM Loop：写入 t_pending_client_call 表（callId 为 UUID，存储 sessionId / agentId / toolId / toolName / params / messageContext / status='pending'），通过 SSE 发送 `{ event: 'client_call', data: { callId, toolName, params } }` 消息，结束本次响应（不继续 LLM 对话）。

#### Scenario: 识别 Client Tool 并挂起
- **GIVEN** LLM 返回 action `{ tool: "client__1__console-log-echo", params: { message: "test" } }`
- **WHEN** 服务端解析 tool 字段发现 `client__` 前缀
- **THEN** 在 t_pending_client_call 表插入一条记录（callId 为新生成的 UUID，status='pending'，messageContext 存储当前 messages 数组）
...
- **WHEN** 服务进程重启
- **THEN** t_pending_client_call 表中的 pending 记录仍然存在，callId 可用于后续恢复
...
接收到浏览器回传的结果后，服务端 MUST 通过 resumeClientResult 方法恢复挂起的 LLM Loop：从 t_pending_client_call 加载 messageContext（messages 数组），拼接新的 observation 消息（content 为工具返回的 result 或 error 的 JSON 字符串），更新 t_pending_client_call 的 status 为 'completed'，继续调用 LLM（将更新后的 messages 发送给 LLM），循环直到返回 final_answer 或再次挂起。

#### Scenario: 恢复并拼接 observation
- **GIVEN** t_pending_client_call 记录中 messageContext 为 `[{ role: 'user', content: '...' }, { role: 'assistant', content: '...', action: {...} }]`
- **AND** 浏览器回传 `{ result: { echo: "test", timestamp: 1718000000000 } }`
- **WHEN** 服务端调用 resumeLlmTurn
- **THEN** 拼接 observation 消息 `{ role: 'observation', content: '{"echo":"test","timestamp":1718000000000}' }` 到 messages 数组末尾
- **AND** 将更新后的 messages 发送给 LLM

#### Scenario: 恢复后继续 Loop
- **GIVEN** LLM 接收到包含 observation 的 messages
- **WHEN** LLM 返回新的响应（可能是 final_answer 或新的 action）
- **THEN** 服务端继续 Loop 逻辑（如返回 final_answer 则结束，如返回新 action 则再次分发）

#### Scenario: 标记 pending 记录为 completed
- **GIVEN** resumeLlmTurn 成功恢复并继续了 Loop
